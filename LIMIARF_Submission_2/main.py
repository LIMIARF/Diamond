...
def main(args):
    ...
    #Train
    class_counts_train = df_train['next_DME_label'].value_counts()
    
    # Extract the counts for class 0 and class 1
    num_class_train0 = class_counts_train.get(0, 0)
    num_class_train1 = class_counts_train.get(1, 0)
    
    # Create the list with the counts
    samples_per_cls_train = [num_class_train0, num_class_train1]
    
    #Val
    class_counts_val = df_val['next_DME_label'].value_counts()
    
    # Extract the counts for class 0 and class 1
    num_class_val0 = class_counts_val.get(0, 0)
    num_class_val1 = class_counts_val.get(1, 0)
    
    # Create the list with the counts
    samples_per_cls_val = [num_class_val0, num_class_val1]
    
    print(f"Number of samples per class in train is {samples_per_cls_train}")
    print(f"Number of samples per class in val is {samples_per_cls_val}")
    ...
    for epoch in range(args.epochs):
        ...
        with torch.enable_grad():
            for batch_idx, data in enumerate(tqdm(train_dataloader)):
                ...
                logits = model(img)
                cb_loss = CB_loss(labels, logits, samples_per_cls_train, 2,loss_type, beta, gamma)
#                 loss = criterion(logits, labels)
        ...
        with torch.no_grad():
            for batch_idx, data in enumerate(val_dataloader):
                img, labels = prepare_data(data, device)
                logits = model(img)
                cb_loss = CB_loss(labels, logits, samples_per_cls_val, 2,loss_type, beta, gamma)
                avg_loss_list.append(cb_loss.item())
...
if __name__ == "__main__":
    ...
    parser.add_argument('--weights_pth_file', type=str, default='./backbone/inception_v3.pth', help='Weights backbone pth file')
